...
public class PayCenterInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
        ...
        if(StringUtils.isNotBlank(imoocUserId) && StringUtils.isNotBlank(password)){
            ...
            if(user == null){
                returnErrorResponse(response, R.errorTokenMsg("用户id或密码不正确！"));
                return false;
            }
            ...
            int days = JamieDateUtils.daysBetween(nowDate, endDate);
            if(days < 0){
                returnErrorResponse(response, R.errorTokenMsg("该账户授权访问日期已失效！"));
                return false;
            }
            ...
            returnErrorResponse(response, new R().errorTokenMsg("请在header中携带支付中心所需的用户id以及密码！"));
            return false;
        }
    ...
    public void returnErrorResponse(HttpServletResponse response, R result) throws IOException,
        ...
        try{
            ...
            out.write(JacksonUtils.objectToJson(result).getBytes("utf-8"));
            out.flush();
        }finally{
            if(out != null){
                out.close();
