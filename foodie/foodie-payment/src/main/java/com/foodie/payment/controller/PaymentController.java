...
import com.foodie.common.utils.CurrencyUtils;
import com.foodie.common.utils.R;
import com.foodie.common.utils.RedisUtils;
import com.foodie.payment.resource.AliPayResource;
import com.foodie.payment.resource.WXPayResource;
import com.foodie.payment.service.PaymentOrderService;
import com.foodie.payment.pojo.Orders;
import com.foodie.payment.pojo.bo.MerchantOrdersBO;
import com.foodie.payment.pojo.vo.PaymentInfoVO;
import com.foodie.payment.wx.entity.PreOrderResult;
import com.foodie.payment.wx.service.WxOrderService;
...
public class PaymentController {
    ...
    public R createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request,
        ...
        if(StringUtils.isBlank(merchantOrderId)){
            return R.errorMsg("参数[orderId]不能为空");
        }
        if(StringUtils.isBlank(merchantUserId)){
            return R.errorMsg("参数[userId]不能为空");
        }
        if(amount == null || amount < 1){
            return R.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
        }
        if(payMethod == null){
            return R.errorMsg("参数[payMethod]不能为空并且不能小于1");
        }
        if(!payMethod.equals(PayMethod.WE_CHAT.type) && !payMethod.equals(PayMethod.ALI_PAY.type)){
            return R.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
        }
        if(StringUtils.isBlank(returnUrl)){
            return R.errorMsg("参数[returnUrl]不能为空");
        }
        ...
        try{
            ...
            e.printStackTrace();
            R.errorException(e.getMessage());
        }

        if(isSuccess){
            return R.ok("商户订单创建成功！");
        }else{
            return R.errorMsg("商户订单创建失败，请重试...");
        }
    }
    ...
    public R getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

        if(StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)){
            return R.errorMsg("查询参数不能为空！");
        }
    ...
    public R getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception {

        //		System.out.println(wxPayResource.toString());

        // 根据订单ID和用户ID查询订单详情
        Orders waitPayOrder = paymentOrderService.queryOrderByStatus(merchantUserId, merchantOrderId,
        ...
        if(waitPayOrder != null){
            ...
            return R.ok(paymentInfoVO);
        }else{
            return R.errorMsg("该订单不存在，或已经支付");
        }
    }
    ...
    public R goAlipay(String merchantOrderId, String merchantUserId) throws Exception {
        ...
        //获得初始化的AlipayClient
        AlipayClient alipayClient = new DefaultAlipayClient(aliPayResource.getGatewayUrl(), aliPayResource.getAppId()
                , aliPayResource
                .getMerchantPrivateKey(), "json", aliPayResource.getCharset(), aliPayResource.getAlipayPublicKey(),
                aliPayResource
                .getSignType());
