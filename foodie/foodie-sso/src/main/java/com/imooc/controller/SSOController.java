...
public class SSOController {
    ...
    public String doLogin(String username, String password, String returnUrl, Model model, HttpServletRequest request
        ...
        redisUtils.set(REDIS_USER_TOKEN + ":" + userResult.getId(), JacksonUtils.objectToJson(usersVO));
    ...
    @ResponseBody
    public R<UserVO> verifyTmpTicket(String tmpTicket, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 验证成功，返回OK，携带用户会话
        return R.ok(JacksonUtils.jsonToPojo(userRedis, UserVO.class));
    }

    @PostMapping("/logout")
    @ResponseBody
    public R<Void> logout(String userId, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        redisUtils.del(REDIS_USER_TICKET + ":" + userTicket);
    ...
    private String createTmpTicket() {
        ...
        try{
            redisUtils.set(REDIS_TMP_TICKET + ":" + tmpTicket, Md5Utils.getMd5Str(tmpTicket), 600);
        }catch(Exception e){
            e.printStackTrace();
        }
        return tmpTicket;
